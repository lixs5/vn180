...
class BacktestingEngine(object):
    ...
    def saveBacktestingResult(self):
        ...
        fig_name = '{0}_{1}_{2}_trade.png'.format(self.stratName, self.symbolName, dt)
        fig_file_name = os.path.abspath(os.path.join(strat_path, fig_name))
        fig = plt.gcf()
        fig.savefig(fig_file_name)
        print (u'图表每笔交易保存至：{0}'.format(fig_file_name))   
       
        txt_name = '{0}_{1}_{2}_trade.txt'.format(self.stratName, self.symbolName, dt)
    ...
    def saveDailyResult(self, df=None, result=None):
        ...
        fig_name = '{0}_{1}_{2}_daily.png'.format(self.stratName, self.symbolName, dt)
        fig_file_name = os.path.abspath(os.path.join(strat_path, fig_name))
        fig = plt.gcf()
        fig.savefig(fig_file_name)
        print (u'图表每笔交易保存至：{0}'.format(fig_file_name))
       
        txt_name = '{0}_{1}_{2}_daily.txt'.format(self.stratName, self.symbolName, dt)
        txt_file_name = os.path.abspath(os.path.join(strat_path, txt_name))
        f = open(txt_file_name,'w')
        f.write(self.outString.encode('utf-8'))
        f.close()
    ...
    def saveParallelOptimization(self, strategyClass, optimizationSetting):
        ...
        txt_name = '{0}_{1}_{2}_optimi.txt'.format(self.stratName, self.symbolName, dt)
        txt_file_name = os.path.abspath(os.path.join(strat_path, txt_name))
        f = open(txt_file_name,'w')
        f.write(self.outString.encode('utf-8'))
        f.close()        
